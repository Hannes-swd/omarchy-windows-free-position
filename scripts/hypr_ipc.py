...
def set_cursor_zoom(factor, timeout=2):
    """Cambia el zoom de toda la pantalla (cursor:zoom_factor). No es un
    dispatcher, hace falta 'hyprctl eval' con hl.config() en vez de dispatch."""
    return subprocess.run(
        ["hyprctl", "eval", f"hl.config({{ cursor = {{ zoom_factor = {float(factor)} }} }})"],
