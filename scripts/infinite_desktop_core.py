...
EV_KEY=1; EV_REL=2; EV_ABS=3; REL_X=0; REL_Y=1; ABS_X=0; ABS_Y=1
...
def _monitor_bounds_from(m):
    # hyprctl monitors reporta ancho/alto en pixeles fisicos del modo de
    # video; las posiciones/tamanos de ventana (hyprctl clients) estan en
    # coordenadas LOGICAS (= pixeles fisicos / scale). Sin esto, el calculo
    # de bordes queda desplazado en monitores con escalado fraccional.
        ...
        if monitors:
            for m in monitors:
                if m.get('focused', False):
                    return _monitor_bounds_from(m)
            return _monitor_bounds_from(monitors[0])
    except:
        pass
...
def get_monitor_center():
    """Devuelve el centro del monitor enfocado (en coordenadas logicas)."""
    b = get_monitor_bounds()
...
def classify_device(path):
    """Devuelve 'mouse', 'touchpad', 'keyboard' o None segun las capacidades reales
    del dispositivo, sin importar el nombre/marca. Esto es lo que permite que
    funcione con cualquier mouse, touchpad o teclado (alambrico, inalambrico, el
    que sea)."""
    try:
    ...
    # EV_ABS entries come back as (code, AbsInfo) tuples, not plain codes.
    abss = set(c[0] if isinstance(c, tuple) else c for c in caps.get(ecodes.EV_ABS, []))

    is_mouse = (ecodes.REL_X in rels and ecodes.REL_Y in rels and ecodes.BTN_LEFT in keys)
    if is_mouse:
        return 'mouse'

    # Touchpads (p.ej. ELAN) reportan la posicion del dedo como coordenadas
    # ABSOLUTAS (ABS_X/ABS_Y), no como movimiento relativo. El "hermano" .../Mouse
    # que muchos touchpads exponen solo declara BTN_LEFT/BTN_RIGHT y rara vez
    # emite REL real al deslizar el dedo, asi que hace falta esta rama aparte.
    is_touchpad = (
        ecodes.ABS_X in abss and ecodes.ABS_Y in abss and ecodes.BTN_TOOL_FINGER in keys
    )
    if is_touchpad:
        return 'touchpad'
...
def touchpad_reader_device(path):
    """Lee eventos de UN touchpad especifico (ABS_X/ABS_Y) y los convierte en
    deltas relativos, igual que mouse_reader_device pero para dispositivos que
    reportan posicion absoluta del dedo en vez de movimiento relativo."""
    global acc_x, acc_y, btn_left, mouse_rel_x, mouse_rel_y
    try:
        fd = open(path, 'rb')
    except Exception:
    ...
    def apply_delta(dx, dy):
        global mouse_rel_x, mouse_rel_y, acc_x, acc_y
        if dx == 0 and dy == 0:
            return
        mouse_rel_x += dx
        mouse_rel_y += dy
    ...
    while True:
        ...
        with lock:
            if etype == EV_KEY and code == BTN_LEFT:
                btn_left = (value == 1)
            elif etype == EV_KEY and code == ecodes.BTN_TOOL_FINGER and value == 0:
                # Dedo levantado: evita un salto al volver a apoyar el dedo
                # en otra posicion de la superficie.
                last_x = None
                last_y = None
            elif etype == EV_ABS and code == ABS_X:
                if last_x is not None:
                    pending_dx += value - last_x
                last_x = value
            elif etype == EV_ABS and code == ABS_Y:
                if last_y is not None:
                    pending_dy += value - last_y
                last_y = value

        if etype == 0:  # SYN_REPORT: un "frame" completo de eventos ABS termino aqui
            with lock:
