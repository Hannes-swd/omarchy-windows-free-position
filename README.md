...
Each screenshot is cached in `~/.cache/omarchy-windows-free-position/window-switcher/`, keyed by window address and its exact position+size. As long as a window hasn't moved or been resized since the last time you opened the switcher, its cached image is reused instantly instead of repeating the move/capture dance — a warm run is on the order of 40x faster than a cold one. This only invalidates on geometry changes, not on content changes: a window that's just had its on-screen content update without moving (a chat scrolling, a page reloading) will show the older screenshot until it's moved, resized, or closed.
